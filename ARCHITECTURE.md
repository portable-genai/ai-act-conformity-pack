# ARCHITECTURE: AI Act Conformity Pack (`ai-act-conformity-pack`)

Hexagonal ports-and-adapters. A pure-stdlib domain core speaks only to ports (`typing.Protocol`s);
adapter families implement them; one env var (`CFP_PROFILE`) swaps the
...
## Request pipeline (`ConformityService.assess`, then the caller)
