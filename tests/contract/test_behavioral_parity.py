...
from conformity_pack.adapters._review_payload import (
    result_to_review,
)
...
def test_the_payload_that_reaches_the_wire_is_redacted_whichever_family_built_it() -> None:
    ...
    service = build_conformity_service(build_container(local_settings()))
...
def test_the_whole_pipeline_answers_on_local_and_fails_fast_on_onprem() -> None:
    ...
    result = build_conformity_service(local).assess(
    ...
    with pytest.raises(NotImplementedError):
        build_conformity_service(onprem).assess(
            sample_cases.ESCALATING_CASE, actor=sample_cases.ACTOR, tenant=sample_cases.TENANT
        )
