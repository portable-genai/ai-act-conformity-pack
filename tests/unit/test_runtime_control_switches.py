...
control this service has: ``CFP_REVIEW_ROUTING`` is read in three states; off binds a
disabled router and says so at startup; on under the managed profile refuses to boot without a
console; and the API, the agent tool and the CLI report ``review_routing`` rather than failing an
already-computed assessment when the console is unreachable.
"""
...
from conformity_pack.adapters.controls import (
    DisabledReviewRouter,
    RecordingReviewRouter,
    ReviewRouting,
)
from conformity_pack.agent import tools
from conformity_pack.api import app as api_module
from conformity_pack.api.app import app
from conformity_pack.cli import main as cli
...
from conformity_pack.envread import ConfiguredEmptyError

_LOOPBACK = ("127.0.0.1", 50000)
_PATH = "/v1/assess"
_ESCALATING: dict[str, Any] = {"system": "credit-decision-copilot"}
_ROUTINE: dict[str, Any] = {"system": "internal-reporting-helper"}
_HEADERS = {"X-Dev-Persona": "auditor"}
_LOCAL_ROUTER = "conformity_pack.adapters.local.review_router.LocalReviewRouter.route"
...
def _managed(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        "conformity_pack.config.resolve_profile",
...
def test_the_off_posture_is_logged_once_however_many_containers(
    ...
    settings = Settings(profile="local", controls=ControlSwitches(review_routing=False))
    with caplog.at_level(logging.WARNING, logger="conformity_pack.config"):
        for _ in range(3):
...
def test_a_failed_hand_off_is_reported_and_logged_never_raised(
    ...
    failed = RecordingReviewRouter(_Refusing())
    with caplog.at_level(logging.WARNING, logger="conformity_pack.adapters.controls"):
...
def test_the_agent_tool_reports_the_hand_off() -> None:
    payload = tools.assess_system("credit-decision-copilot", tenant="demo-bank")
...
def test_the_cli_reports_the_hand_off(
    ...
    monkeypatch.setenv(REVIEW_ROUTING_ENV, "off")
    assert cli.main(["assess", "credit-decision-copilot"]) == 0
