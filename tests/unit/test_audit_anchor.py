...
from conformity_pack.adapters.local.audit import (
    LocalAuditAdapter,
)
...
from conformity_pack.ports.audit import AuditSinkPort

from tests.fixtures import sample_cases

_ANCHOR_ENV = "CFP_AUDIT_ANCHOR"

#: Systems to record, one per tier band, so the anchor witnesses a realistic run.
_SYSTEMS = (
    sample_cases.ESCALATING_SYSTEM,
    sample_cases.ROUTINE_SYSTEM,
    sample_cases.PII_SYSTEM,
)


def _service_over(audit: AuditSinkPort, settings: Settings) -> ConformityService:
    """A conformity service using the GIVEN audit adapter, other ports from a fresh container."""
...
def _record_three(settings: Settings) -> LocalAuditAdapter:
    ...
    service = _service_over(audit, settings)
    for system in _SYSTEMS:
        service.assess(
            AiSystemInput(system=system), actor=sample_cases.ACTOR, tenant=sample_cases.TENANT
        )
    return audit
...
def test_an_append_after_truncation_cannot_relaunder_the_anchor(tmp_path: Path) -> None:
    ...
    with pytest.raises(AuditChainError):
        _service_over(audit, settings).assess(
            AiSystemInput(system=sample_cases.ESCALATING_SYSTEM),
