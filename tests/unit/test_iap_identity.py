...
from conformity_pack.adapters.gcp.identity import (
    _AUDIENCE_ENV,
...
from conformity_pack.ports.identity import (
    VERIFIED,
...
_API_MODULE = "conformity_pack.api.app"
...
def _data_port_lines(port: str, module: str, cls: str) -> list[str]:
    """One port's adapters block with local/gcp both bound to the SDK-free local adapter.

    Kept short by construction so no rendered line depends on the package name's length: the
    dotted target is assembled from ``_PKG`` and the arguments rather than written inline.
    """
    local = f"{_PKG}.adapters.local.{module}:{cls}"
    onprem = f"{_PKG}.adapters.onprem.{module}:OnPrem{cls.removeprefix('Local')}"
    return [
        f"  {port}:",
        f"    local: {local}",
        f"    gcp: {local}",
...
_REBOUND_SETTINGS = "\n".join(
    [
        ...
        *[f"    {p}: {_PKG}.adapters.local.audit:LocalAuditAdapter" for p in ("local", "gcp")],
        ...
        # The gcp IDENTITY binding is the real subject of these tests, so it is the one port NOT
        # rebound to a local adapter.
        "  identity:",
        ...
        # Every other port is rebound to the SDK-free local adapter, so these identity tests need
        # no live registry, obligations feed, evidence surface, model, KB or BigQuery. Every port
        # must appear or the loader refuses the partial block.
        *_data_port_lines("review_router", "review_router", "LocalReviewRouter"),
        *_data_port_lines("registry", "registry", "LocalRegistryAdapter"),
        *_data_port_lines("obligations", "obligations", "LocalObligationsAdapter"),
        *_data_port_lines("evidence", "evidence", "LocalEvidenceAdapter"),
        *_data_port_lines("narrator", "narrator", "LocalNarratorAdapter"),
        *_data_port_lines("retrieval", "retrieval", "LocalRetrievalAdapter"),
        *_data_port_lines("horizon", "horizon", "LocalHorizonAdapter"),
        *_data_port_lines("matrix_store", "matrix_store", "LocalMatrixStore"),
...
def test_a_malformed_assertion_from_a_lan_peer_answers_401_and_never_500(
    ...
    with TestClient(module.app, client=(LAN_PEER, 50000)) as client:
        resp = client.post(
            "/v1/assess",
            json={"system": "credit-decision-copilot"},
