...
def redacted_citations(citations: tuple[Citation, ...]) -> tuple[Citation, ...]:
    ...
    A snippet is a slice of its source (``AiSystemCard.description[:80]``) and a locator is
    routinely built from one (``hrz3:agent:<the card name>``, with the same name repeated in the
    title), so all three are upstream free text wearing a structural-looking name. No sink can
    tell an engine-built citation from a registry-built one by inspection, so this masks
