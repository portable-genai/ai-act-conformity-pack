...
``CFP_PROFILE`` is NO CHOICE rather than a silent ``local``. Every
...
``CFP_S2S_TOKEN`` is wrong in the most dangerous direction:
...
_TOKEN_ENV = "CFP_S2S_TOKEN"
_INSECURE_DEMO_ENV = "CFP_ALLOW_INSECURE_DEMO"
_CORS_ORIGINS_ENV = "CFP_CORS_ORIGINS"
_ALLOWED_CALLERS_ENV = "CFP_S2S_ALLOWED_CALLERS"
_AUDIENCE_ENV = "CFP_S2S_AUDIENCE"
_API_HOST_ENV = "CFP_API_HOST"
...
def require_service_caller(request: Request) -> None:
    ...
    stays OPEN when ``CFP_S2S_TOKEN`` is unset (loopback dev with zero
    secrets), so an UNSET ``CFP_PROFILE`` must never be allowed to
    ...
    if not _request_choice(request).service_auth_configured:
        raise HTTPException(
            ...
            detail=(
                ...
                "CFP_PROFILE is not set, so no authentication scheme "
                "has been chosen"
            ),
        )
...
_SERVICE_NAME = "ai-act-conformity-pack"
...
_APP_TITLE = "AI Act Conformity Pack"
...
# configuration says would. A request that noted nothing sends
...
#
# Note what is NOT in this expression: CFP_S2S_TOKEN. A service
...
def _service(container: Container) -> ConformityService:
    """Build the conformity service from the container's bound ports (never at import)."""
...
@app.post("/v1/assess", response_model=AssessResponse, tags=["artifacts"])
def assess(
    ...
    try:
        result = _service(container).assess(
            AiSystemInput(system=request.system),
            actor=principal.actor,
            tenant=principal.tenant,
            as_of=request.as_of,
        )
    except TenantScopeError as exc:
        # 403, not 404 and not 500. The caller authenticated and the system exists; the verified
        # identity simply carries no tenant partition (an IAP assertion with no `hd` claim
