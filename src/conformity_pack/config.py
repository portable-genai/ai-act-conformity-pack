...
One env var (``CFP_PROFILE``) selects the adapter family for every
...
from .ports.horizon import HorizonPort
...
_PROFILE_ENV = "CFP_PROFILE"
_SETTINGS_ENV = "CFP_SETTINGS"
...
_GENERATOR_MODEL_ATTR: str = "narrator_model"
...
class ProfileChoice:
    ...
    The variable is ``CFP_PROFILE``, named once in
    :data:`_PROFILE_ENV`.
    ...
    ``os.environ.get("CFP_PROFILE", "local")``: that fallback reads an
...
DEFAULT_BINDINGS: dict[str, dict[str, str]] = {
    ...
    "registry": {
        "local": f"{_PKG}.adapters.local.registry:LocalRegistryAdapter",
        "gcp": f"{_PKG}.adapters.gcp.registry:CloudRegistryAdapter",
        "onprem": f"{_PKG}.adapters.onprem.registry:OnPremRegistryAdapter",
    },
    "obligations": {
    ...
    "evidence": {
        "local": f"{_PKG}.adapters.local.evidence:LocalEvidenceAdapter",
        "gcp": f"{_PKG}.adapters.gcp.evidence:CloudEvidenceAdapter",
        "onprem": f"{_PKG}.adapters.onprem.evidence:OnPremEvidenceAdapter",
    },
    "narrator": {
        "local": f"{_PKG}.adapters.local.narrator:LocalNarratorAdapter",
        "gcp": f"{_PKG}.adapters.gcp.narrator:CloudNarratorAdapter",
        "onprem": f"{_PKG}.adapters.onprem.narrator:OnPremNarratorAdapter",
    },
    "retrieval": {
        "local": f"{_PKG}.adapters.local.retrieval:LocalRetrievalAdapter",
        "gcp": f"{_PKG}.adapters.gcp.retrieval:CloudRetrievalAdapter",
    ...
    "matrix_store": {
        "local": f"{_PKG}.adapters.local.matrix_store:LocalMatrixStore",
        "gcp": f"{_PKG}.adapters.gcp.matrix_store:CloudMatrixStore",
        "onprem": f"{_PKG}.adapters.onprem.matrix_store:OnPremMatrixStore",
    },
}
...
def _read_settings_file(path: Path | None = None) -> dict[str, Any]:
    ...
    An EXPLICIT path (argument or ``CFP_SETTINGS``) that does not
...
REVIEW_ROUTING_ENV = "CFP_REVIEW_ROUTING"
...
class Container:
    ...
    @cached_property
    def registry(self) -> RegistryPort:
        adapter = self._bind("registry")
        assert isinstance(adapter, RegistryPort)
        return adapter

    @cached_property
    def obligations(self) -> ObligationsPort:
        adapter = self._bind("obligations")
        assert isinstance(adapter, ObligationsPort)
        return adapter

    @cached_property
    def evidence(self) -> EvidencePort:
        adapter = self._bind("evidence")
    ...
    @cached_property
    def horizon(self) -> HorizonPort:
        adapter = self._bind("horizon")
        assert isinstance(adapter, HorizonPort)
        return adapter

    @cached_property
    def matrix_store(self) -> MatrixStorePort:
        adapter = self._bind("matrix_store")
        assert isinstance(adapter, MatrixStorePort)
        return adapter
