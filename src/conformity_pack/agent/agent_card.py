...
class AgentCard:
    ...
    provider: str = "ai-act-conformity-pack"
...
SKILLS: tuple[AgentSkill, ...] = (
    AgentSkill(
        id="assess_system",
        name="AI-system conformity assessment",
        description=(
            "Classify one deployed AI system's EU AI Act risk tier, compute its "
            "obligation-applicability matrix and score evidence sufficiency (all deterministic "
            "engines), record an already-redacted audit event, and ROUTE the result to human "
            "review when it is consequential (rule R8). The tier and matrix are computed by pure "
...
def _resolve_url(settings: Settings) -> str:
    ...
    return f"https://ai-act-conformity-pack.{settings.region}.internal.example/a2a"
