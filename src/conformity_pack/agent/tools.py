...
DEFAULT_ACTOR = "ai-act-conformity-pack-agent"
...
def assess_system(
    ...
    """Assess one AI system for conformity and route it for review when consequential.

    Classifies the system's EU AI Act risk tier, computes the obligation-applicability matrix and
    scores evidence sufficiency (all pure deterministic engines), writes an already-redacted
    audit event, and, when the result is consequential (high-risk/prohibited tier, an undecided
    cell, or a named evidence gap), submits it to the human-review console (rule R8).

    Args:
      system: The registered AI-system name to assess.
      actor: The verified identity this call is attributed to.
      tenant: Tenant partition: asserted on an outbound review, and the key the persisted
        matrix is stored under. Supplying ``as_of`` without it is refused rather than writing
        lineage into the partition every unscoped caller shares.
      as_of: Replay date stamped on the persisted applicability matrix.
    ...
    if not isinstance(payload, dict):  # pragma: no cover - dataclasses serialise to objects
        raise TypeError("a conformity result must serialise to a JSON object")
...
TOOL_FUNCTIONS = (assess_system, verify_audit_trail)
