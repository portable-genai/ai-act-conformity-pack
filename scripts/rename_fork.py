...
The catalog id (``Rgc14``) is left alone unless ``--catalog-id`` is given: it names the entry in
...
_OLD_PACKAGE = "conformity_pack"
_OLD_ENV_PREFIX = "CFP_"
_OLD_BARE_PREFIX = "CFP"
_OLD_RESOURCE = "rgc14-svc"
_OLD_DIST = "ai-act-conformity-pack"
_OLD_CATALOG_ID = "Rgc14"
...
def _rewrite_text(text: str, args: argparse.Namespace) -> tuple[str, int]:
    ...
    # The env prefix, only where it PREFIXES an upper token (CFP_PROFILE), never mid-word.
...
def main() -> int:
    ...
    ap.add_argument(
        "--resource",
        required=True,
        help="new cloud resource stem (Terraform name_prefix), e.g. acme-aiact",
    )
