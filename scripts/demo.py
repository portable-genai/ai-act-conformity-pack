...
from conformity_pack.domain import (
    kernel,
    models,
)
...
from conformity_pack.domain.pii import (
    JURISDICTIONS,
)
...
SERVICE_NAME = "AI Act Conformity Pack"
CATALOG_ID = "ai-act-conformity-pack"
REPOSITORY = "ai-act-conformity-pack"
...
#: than trusting the pattern pack to agree with itself. It is carried on the gamma system's
#: registry-card description, so the redact-before-audit step has a real identifier to mask.
PLANTED_NRIC = "S1234567D"


@dataclass(frozen=True, slots=True)
class DemoCase:
    """One demo case: the fictional narrative shown on screen plus the fleet system to assess.

    ``subject`` and ``text`` are display flavour (fictional parties, ``.example`` domains, RFC
    5737 / RFC 3849 literals); ``system`` is the registered fixture-fleet system the deterministic
...
ROUTINE_CASE = DemoCase(
    ...
    text="internal reporting helper reviewed from 192.0.2.10; mail ops@beta.example",
    system="internal-reporting-helper",
)

ESCALATING_CASE = DemoCase(
    subject="Acme Holdings (FICTIONAL)",
    text="credit-decision copilot reviewed from 192.0.2.20; mail ops@acme.example",
    system="credit-decision-copilot",
)

PII_CASE = DemoCase(
    subject="Gamma LLP (FICTIONAL)",
    text=(
        "gamma analytics copilot: NRIC "
        + PLANTED_NRIC
        + " and mail ops@gamma.example seen on host 2001:db8::7"
    ),
    system="gamma-analytics-copilot",
)
...
class DemoRun:
    ...
    def __init__(self, workdir: Path | None = None) -> None:
        ...
        self.service = ConformityService(
            audit=self.container.audit,
            registry=self.container.registry,
            obligations=self.container.obligations,
            evidence=self.container.evidence,
    ...
    def _triage_panels(self, case: DemoCase, *, expect_routing: bool) -> Produced:
        result = self.service.assess(
            AiSystemInput(system=case.system), actor=ACTOR, tenant=TENANT, as_of="2026-08-08"
        )
        self.cases += 1
        review_ref = ""
        ...
        decision = Panel(
            title="Classification: " + result.subject,
            rows=(
                Row(
                    "EU AI Act risk tier",
                    result.tier.value + (" (conditional)" if result.conditional else ""),
                    "bad" if result.requires_human_review else "ok",
                ),
                Row("Obligations applying", str(result.applies_count)),
                Row("Evidence gaps", str(len(result.gaps))),
                Row("Requires human review", str(result.requires_human_review)),
                Row(
                    "Routed to review",
                    review_ref or "not routed (minimal risk)",
                    "ok" if consistent else "bad",
                ),
                Row("Attributed to", ACTOR),
            ),
            note=(
                "The tier and the matrix are computed by pure stdlib code and are replayable. A "
                "model narrates this result; it never produces it."
            ),
            tone="ok" if consistent else "bad",
        )
        matrix_rows = tuple(
            Row(cell.obligation_id + " (" + cell.framework + ")", cell.applicability.value)
            for cell in result.applicability
        )
        matrix = Panel(
            title="Obligation-applicability matrix",
            rows=matrix_rows or (Row("obligations", "none", "warn"),),
            note="One cell per obligation: APPLIES, NOT_APPLICABLE or CONDITIONAL, with reasons.",
        )
        evidence = Panel(
            title="Evidence",
            rows=tuple(
        ...
        return [decision, matrix, evidence], facts
...
def _exit_audit(container: Any) -> Any:
    return container.audit.record(
        kernel.AuditEvent(
            ...
            redacted_summary="credit-decision-copilot: high-risk classification",
        )
    )
...
def _exit_review(container: Any) -> Any:
    citation = kernel.Citation(
        source_id="hrz3:agent:credit-decision-copilot",
        title="Registry card",
        snippet="credit-scoring",
    )
    return container.review_router.route(
        models.ConformityResult(
            subject=ESCALATING_CASE.system,
            severity=kernel.Severity.HIGH,
            decision=kernel.Decision.ESCALATED,
            summary="credit-decision-copilot: high-risk classification",
            requires_human_review=True,
            citations=(citation,),
            tier=models.RiskTier.HIGH,
        ),
        maker=ACTOR,
        tenant=TENANT,
    )
...
def _exit_registry(container: Any) -> Any:
    return container.registry.list()


def _exit_obligations(container: Any) -> Any:
    return container.obligations.obligations()


def _exit_evidence(container: Any) -> Any:
    return container.evidence.harvest(ESCALATING_CASE.system)


def _exit_narrator(container: Any) -> Any:
    return container.narrator.narrate("FACTS:\n{}\nGROUNDING:\n- rule")


def _exit_retrieval(container: Any) -> Any:
    return container.retrieval.search("high eu-ai-act")


def _exit_horizon(container: Any) -> Any:
    return container.horizon.changes()


def _exit_matrix_store(container: Any) -> Any:
...
EXIT_CALLS: dict[str, Callable[[Any], Any]] = {
    ...
    "matrix_store": _exit_matrix_store,
}
